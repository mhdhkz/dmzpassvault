...
document.addEventListener('DOMContentLoaded', function (e) {
  ...
  if (dt_user_table) {
          ...
          render: (data, type, full, meta) => {
            return `
              <div class="d-flex align-items-center">
                  <a href="javascript:;" class="btn btn-sm btn-primary text-white d-flex align-items-center gap-1 btn-generate-password" data-id="${full.id}" title="Request">
                    <i class="bx bx-key"></i> <small>Request</small>
                  </a>
                </div>
            `;
          }
    ...
    // Global delegation untuk tombol generate-password, termasuk di modal
    document.addEventListener('click', function (e) {
      const btn = e.target.closest('.btn-generate-password');
      if (btn) {
        const id = btn.getAttribute('data-id');
        if (id) {
          fetch(`/identity/${id}/generate-password`, {
            method: 'POST',
            headers: {
            ...
            .then(data => {
              if (data.success) {
                Swal.fire('Sukses', 'Password berhasil dibuat ulang', 'success');
              } else {
